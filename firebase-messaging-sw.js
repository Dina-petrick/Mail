...
const addNotificationId = (db, notificationId) => {
    return new Promise((resolve, reject) => {
        ...
        getRequest.onsuccess = (event) => {
            const existingId = event.target.result;

            if (!existingId) {
                objectStore.add({ id: notificationId });
                console.log(`Notification with ID ${notificationId} added.`);
            } else {
                console.log(`Notification with ID ${notificationId} already exists. Skipping...`);
            }
            resolve();
        };
...
const getAllNotificationIds = (db) => {
    return new Promise((resolve, reject) => {
        ...
        getAllRequest.onsuccess = (event) => {
            const ids = event.target.result.map(item => item.id);
            resolve(ids);
        };
...
const handleNotification = async (notificationId) => {
    try {
        ...
        await addNotificationId(db, notificationId);
    } catch (error) {
...
let respondToServer;
try {
    respondToServer = async (_payload, messageId, isConversion, smartCode) => {
        const isConversionValue = typeof isConversion === "string" ? isConversion : "false";
        const smartCodeValue = typeof smartCode === "string" ? smartCode : "";
    
        try {
            const [passportId, profileID, tenantId] = await Promise.all([
            ...
            const db = await openDatabase();
            const existingIds = await getAllNotificationIds(db); 
    
            if (existingIds.includes(messageId)) {
                console.log(`Notification ID ${messageId} already exists. Skipping fetch...`);
            } else {
                const data = await getBaseUrl(campaignTracking);
                if (data.domainName) {
                ...
                const postData = async (data, option) => {
                    try {
                        ...
                        if (res && !res.status ) {
                            console.log("Retrying postData...");
                            await new Promise(resolve => setTimeout(resolve, 1500)); // Wait for 1.5 seconds
                            return postData(data, option); // Retry postData
                        }

                        console.log("payload.status:", payload.status);
    
                        if(payload.status == '5'){
                            await handleNotification(messageId);
                        } else {
                            console.log("Notification ID not added to IndexedDB ", "& payload.status :: " , payload.status);
                        }
                ...
                await postData(data, option);
            }
        } catch (error) {
            console.error('Error responding to server:', error);
        }
    };
    
} catch (error) {
    console.error('Error defining respondToServer:', error);
    respondToServer = function (_payload, messageId, isConversion, smartCode) {
...
(async () => {
    try {
        const db = await openDatabase();
        const ids = await getAllNotificationIds(db);
        console.log('Stored Notification IDs:', ids);
    } catch (error) {
...
async function handlePush(event) {
    try {
        let payload = event.data.json();
        console.log("sw: handlePush", payload);
        dataforlater = payload;
    ...
    try {
        ...
        if ((("IsCarousel" in payload.data) || ("inAppNotification" in payload.data)) && ((payload.data.inAppNotification == "true" || payload.data.inAppNotification == true) || (payload.data.IsCarousel == "true" || payload.data.IsCarousel == true))) {
            try {
                ...
                try {
                    if (existingIds.includes(payload.data.id)) {
                        console.log(`Notification ID ${payload.data.id} already exists.`)
                    } else {
...
self.onnotificationclick = (event) => {
    console.log(event , "sw Event onNotifciation Click")
    const clickedNotification = event.notification;
    let messageId = clickedNotification.tag;
